...
AccOut='/Users/junaid/Desktop/DSCN/CMNT_RSA/new_timing_files/good_accuracy/'
AccOut2='/Users/junaid/Desktop/DSCN/CMNT_RSA/new_timing_files/good_accuracy_combined_runs/'
CombOut='/Users/junaid/Desktop/DSCN/CMNT_RSA/new_timing_files/combined_runs/'
...
writeTimeFiles = function(infile, outdir) {
  ...
  for (c in 1:4) {
    ...
    Tmp = Data[Data$MentalState == Conds2[c] & Data$ACC == 1, ]
  ...
  # if there are inaccurate responses
  if (sum(Data$ACC==0) > 0){
    Tmp = Data[Data$ACC == 0, ]
    BTimes=paste(Tmp$CumulativeTrialStart,':',Tmp$ButtonPressTime,sep="")
    write(BTimes, paste(outdir, 'Wrong_run-0', r, '.txt', sep = ""), ncolumns=length(BTimes))
  }
...
for (s in 1:length(Subs$V1)){
  ...
  if ( !dir.exists(paste(AccOut,Subs$V1[s],sep=""))){
    dir.create(paste(AccOut,Subs$V1[s],sep=""))
  }
  
  # define output directory
  OutDir=paste(AccOut,Subs$V1[s],'/',sep="")
